...
class KucoinAPIClient {
  constructor() {
    this.headers = {
      ...
      Referer:
        "https://www.kucoin.com/miniapp/tap-game?inviterUserId=376905749&rcode=QBSLTEH5",
      "Sec-Ch-Ua":
  ...
  async increaseGold(cookie, increment, molecule) {
    const url =
      "https://www.kucoin.com/_api/xkucoin/platform-telebot/game/gold/increase?lang=en_US";
    ...
    while (true) {
      for (let i = 0; i < cookies.length; i++) {
        ...
        for (let j = 0; j < points.length; j++) {
          ...
          const result = await this.increaseGold(
            cookie,
            increment,
            currentMolecule
          );
          if (result.success) {
            this.log(
              `Cho ăn thành công, đã bón được ${result.data.data} sâu`,
              "success"
            );
